...
const gridSize = Math.cbrt(totalNodes); // Вычисляем размер стороны куба
const spacing = 10; // Расстояние между узлами
const offset = (gridSize - 1) * spacing / 2; // Смещение для центрирования куба

// Определяем координаты узла в 3D
const xIndex = index % gridSize; // Положение узла по оси X
const yIndex = Math.floor((index / gridSize) % gridSize); // Положение по оси Y
const zIndex = Math.floor(index / (gridSize * gridSize)); // Положение по оси Z

// Преобразуем в координаты
const x = xIndex * spacing - offset; // Смещение по оси X
const y = yIndex * spacing - offset; // Смещение по оси Y
const z = zIndex * spacing - offset; // Смещение по оси Z
  ...
  if (node.labels) {
    if (node.labels.includes('Group')) {
      color = 0x0000FF; //  типа "Group"
    } else if (node.labels.includes('User')) {
      color = 0x00FF00; // Нтипа "User"
    }
  }
...
document.getElementById('addConnectionBtn').addEventListener('click', function() {
  ...
  listItem.innerHTML = `
    ...
    <button class="removeConnectionBtn">Убрать</button>
  `;
